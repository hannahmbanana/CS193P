...
- (void) updateUI
{
  ...
  for (UIButton *cardButton in self.buttonGridView.cardButtonArray) {
    ...
    NSAttributedString *attributedCardString = [[NSAttributedString alloc] initWithString:[numberShapeString copy]
                                                                               attributes:attributes];
    
    // set to be the card's title
    if (!card.isMatched) {
      [cardButton setAttributedTitle:attributedCardString forState:UIControlStateNormal];
